...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Reputation.metadata" });
...
export default async function ReputationPage({
  ...
  const gbpReviewLink = "https://search.google.com/local/writereview?placeid=ChIJBx3mP_mOzhUR79S06K7h6Sg" // Placeholder
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="text-left">
                    <span className="block text-xs font-bold uppercase tracking-widest text-primary mb-1">{t("card.step")}</span>
                    <span className="text-2xl font-bold block">{t("card.title")}</span>
                    <span className="text-sm text-muted-foreground">{t("card.subtitle")}</span>
                  </div>
